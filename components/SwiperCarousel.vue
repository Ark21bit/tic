...
<div class="col-[full] relative py-[30px] lg:pt-[60px] lg:pb-[45px]">  
  <div class="w-full h-full bg-black absolute col-[full] top-0 left-0">
    <swiper class="w-full h-full " :loop="true" :modules="[SwiperThumbs, SwiperEffectFade]" :effect="'fade'" :allowTouchMove="false" @swiper="setThumbsSwiper">      
      <SwiperSlide v-for="item, index in data" class="relative">          
          <div v-html="item.media_preview" class="w-full h-full [&>img]:w-full [&>img]:h-full [&>img]:object-cover [&>img]:blur [&>img]:scale-[1.05]"></div>         
      </SwiperSlide>      
  </swiper>
  </div>
  <swiper @swiper="setSwiper" :thumbs="{ swiper: thumbsSwiper }" :loop="true" class="wrapper" :modules="[SwiperThumbs, SwiperNavigation, EffectCarousel]" :slidesPerView="'auto'" :effect="'carousel'" :navigation="{ prevEl: '.prev', nextEl:'.next' }" >
      <SwiperSlide class="cursor-pointer relative"  v-for="item, index in data" @click="slideToIndex(index)">
         <Card textClass="lg:text-2xl text-base" :title="item.lang_info.title" :img="item.media_preview" :is-sale="item.is_sale" :slug="item.target_url" textSaleClass="lg:text-base text-sm" class="h-[360px] w-[290px] lg:h-[499px] lg:w-[487px]"/>
      </SwiperSlide>
