<template>
    <Dropdown class="border-b-fline grow" active-class="border-b" v-if="children.length > 0" :key="link.id" >
        ...
        <ul class="flex py-5 flex-col text-fblack gap-[15px]">           
            <li v-for="(item) in children" class="before:w-1.5 before:h-1.5 before:rounded-full before:bg-fred flex items-start before:mt-[.5em] before:shrink-0 gap-2.5">
