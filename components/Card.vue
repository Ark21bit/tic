<template>
    <div class="relative flex gap-4 flex-col justify-end items-start rounded-[10px] p-[15px] lg:p-5 overflow-hidden">
        <img src="@/assets/imgs/cards/img1.png" alt="" class="absolute object-cover top-0 left-0 w-full h-full">
        <div v-if="props.isSale" class="absolute rounded-tl-[10px] rounded-br-[10px] top-0 left-0 text-white bg-fred py-2 px-6 font-semibold uppercase">sale</div>
        <p class="z-10 lg:text-lg text-base text-white font-medium">Дервний город Болгар</p>
        <p class="z-10 px-2.5 py-[5px] rounded-[5px] bg-[rgba(231,54,61,0.8)] text-white text-xs font-medium">От 1000 руб.  / чел</p>
        <Button to="/" tag="router-link" color="secondary" size="L" class="z-10 max-lg:w-full">Подробнее</Button>       
    </div>
</template>

<script setup>

const props = defineProps(
    {
        isSale: { type: Boolean, default:false }, 
    }
)

</script>

