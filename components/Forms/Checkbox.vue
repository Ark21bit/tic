<template>
    <label class="grid gap-[14px] text-sm grid-cols-[auto_1fr] gap-y-[5px] group items-start cursor-pointer" :class="{ 'error text-fred':errorTitle }">
        ...
        <slot/>
        <slot name="error">
            <span v-if="errorTitle" class="text-fred col-start-2">{{ errorTitle }}</span>
        </slot>
    </label>
</template>
